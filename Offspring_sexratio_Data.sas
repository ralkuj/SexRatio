...
*	Date:		2020-01-29
*	Purpose:	Create data for analysis of sex of offspring between parental siblings.
*********************************************************************************;
...
PROC EXPORT DATA= WORK.family 
            OUTFILE= "~\Familiality_of_offspring_sex_cohort_20190116.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= WORK.pairsexport 
            OUTFILE= "~\Familiality_of_offspring_sex_cousinpairs_20190116.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
