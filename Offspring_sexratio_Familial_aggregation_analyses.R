...
### Restrict data to half sibs only
dat <- dat[ dat$sibtype==2 , c('konbarn','konbarn2','KON','KON2','FODELSEDATUM','FODELSEDATUM2','nrbornsamedate','nrbornsamedate2','bnr','bnr2' , 'LOPNRFAR' ) ]

### Remove twins in offspring generation
dat <- dat[ dat$nrbornsamedate==1 & dat$nrbornsamedate2==1 , ]

### GEE Analysis
fitHP <- summary(gee( konbarn ~ konbarn2 , data=dat , link = 'logit' , clusterid = 'LOPNRFAR' ) )$coef

### Analyses of first-born only
fitHPfirst <- summary( gee( konbarn ~ konbarn2 , data=dat[ dat$bnr==1&dat$bnr2==1 , ] ,  link='logit' , clusterid = 'LOPNRFAR' ) )$coef
